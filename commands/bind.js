...
exports.run = async (client, message, args) => {
	...
	if (setup == false){
		...
		// fetch data
		await axios.get(`https://groups.roblox.com/v1/groups/${args[1]}`)
			.then(function (response) {
				groupID = Number(args[1]);
				group_name = response.data.name
				owner_id = response.data.owner.userId;
			});

    await axios.get(`https://groups.roblox.com/v1/groups/${args[1]}/roles`)
      .then(function (response) {
