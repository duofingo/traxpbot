...
exports.run = async (client, message, args, groupID) => {
    ...
    var rankBadEmbed = new Discord.MessageEmbed()
			.setColor(0xf54242)
			.setDescription(`\`${rblx_username}\` is not an Immune!`)
    ...
    await axios.get(`https://groups.roblox.com/v2/users/${rblx_id}/groups/roles`)
		...
		if (flag){
			var badEmbed = new Discord.MessageEmbed()
				.setColor(0xf54242)
				.setDescription(`I can't find **${rblx_username}** in ROBLOX. It seems they don't exist!`)
    ...
    if (dbFlag == true) {
      ...
      await axios.get(`${client.config.firebase_url}/guilds/${message.guild.id}/users/${rblx_id}.json`).then(function (response) {
          if (response.data){
          newXP_Value = response.data.oldXp;
          rankBeforeImmune = response.data.currentRank
          }
       });
      ...
      var demotionEmbed = new Discord.MessageEmbed()
					.setColor(0xf54242)
					.setImage("https://media.giphy.com/media/ehhuGD0nByYxO/giphy.gif")
					.setDescription(`:warning: **\`${rblx_username}\` has been declared __vulnerable__ to the Flare Virus! :warning:**`)
			
			await rblxFunctions.setRank({ group: groupID, target: rblx_id, rank: rankBeforeImmune});
      message.channel.send(demotionEmbed);
      newXP_Value = 0
  }}
...
exports.info = {
    names: ["exileimmune"],
    usage: 'exileimmune <rblx_username>',
    description: "Declare user as not Immune to Flare."
};