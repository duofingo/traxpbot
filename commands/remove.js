...
exports.run = async (client, message, args, groupID) => {
	...
	if (!message.member.roles.cache.some(role => role.name === `${client.config.xp_role}`)) {
		return message.channel.send(`Sorry ${message.author}, but only users with the **${client.config.xp_role}** role can run that command!`).then(message => message.delete({timeout: 5000, reason: "delete"}));
	};
    ...
    //checks if a user is blacklisted. Cannot give blacklisted individuals experience now.
		await axios.get(`${client.config.firebase_url}/guilds/${message.guild.id}/blacklist/${rblx_id}.json`)
			.then(function (response) {
				if (response.data != null){
					blacklisted = true
					var badEmbed2 = new Discord.MessageEmbed()
					.setColor(0xf54242)
					.setDescription(`User **${rblx_username}** is blacklisted!\n\nReason:\n> **${response.data.description}**`)
					return(message.channel.send(badEmbed2));
				}
			})
		...
		await axios.get(`https://groups.roblox.com/v2/users/${rblx_id}/groups/roles`)
			...
			if (previous_rolesetID >= 1 && client.config.demoteUsers == true) {
				...
				if (previousRank_xp !== -1 && blacklisted != true) {
